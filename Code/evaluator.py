...
def evaluate(ast, environment, watch_var=None, source_code=None):
    if ast["tag"] == "number":
    ...
    if ast["tag"] == "list":
        items = []
        for item in ast["items"]:
            result, item_status = evaluate(item, environment, watch_var, source_code)
    ...
    if ast["tag"] == "object":
        object = {}
        for item in ast["items"]:
            key, key_status = evaluate(item["key"], environment, watch_var, source_code)
            if key_status == "exit": return key, "exit"
            assert type(key) is str, "Object key must be a string"
            value, value_status = evaluate(item["value"], environment, watch_var, source_code)
    ...
    if ast["tag"] == "identifier":
        ...
        if "$parent" in environment:
            return evaluate(ast, environment["$parent"], watch_var, source_code)
    ...
    if ast["tag"] == "+":
        left_value, l_status = evaluate(ast["left"], environment, watch_var, source_code)
        if l_status == "exit": return left_value, "exit"
        right_value, r_status = evaluate(ast["right"], environment, watch_var, source_code)
    ...
    if ast["tag"] == "negate":
        value, status = evaluate(ast["value"], environment, watch_var, source_code)
    ...
    if ast["tag"] == "assert":
        if ast["condition"]:
            condition_value, cond_status = evaluate(ast["condition"], environment, watch_var, source_code)
            ...
            if not is_truthy(condition_value):
                ...
                if "explanation" in ast and ast["explanation"]:
                    explanation_val, expl_status = evaluate(ast["explanation"], environment, watch_var, source_code)
    ...
    if ast["tag"] == "if":
        ...
        if is_truthy(condition_value):
            val, status = evaluate(ast["then"], environment, watch_var, source_code)
            if status: # Propagate "return", "exit", "break", "continue"
                return val, status
        else:
            if "else" in ast:
                val, status = evaluate(ast["else"], environment, watch_var, source_code)
                if status: # Propagate "return", "exit", "break", "continue"
                    return val, status
    ...
    if ast["tag"] == "while":
        ...
        while is_truthy(condition_value):
            val, body_status = evaluate(ast["do"], environment, watch_var, source_code)
    ...
    if ast["tag"] == "statement_list":
        ...
        for statement in ast["statements"]:
            last_value, status = evaluate(statement, environment, watch_var, source_code)
    ...
    if ast["tag"] == "program":
        last_value = None
        for statement in ast["statements"]:
            val, status = evaluate(statement, environment, watch_var, source_code)
            if status:
                if status == "return":
    ...
    if ast["tag"] == "call":
        function, func_status = evaluate(ast["function"], environment, watch_var, source_code)
        if func_status == "exit": return function, "exit"
        argument_values = []
        for arg in ast["arguments"]:
            arg_val, arg_status = evaluate(arg, environment, watch_var, source_code)
        ...
        val, status = evaluate(function["body"], local_environment, watch_var, source_code)
    ...
    if ast["tag"] == "complex":
        base, base_status = evaluate(ast["base"], environment, watch_var, source_code)
        if base_status == "exit": return base, "exit"
        index, index_status = evaluate(ast["index"], environment, watch_var, source_code)
    ...
    if ast["tag"] == "assign":
        ...
        if target["tag"] == "identifier":
            ...
            base, base_status = evaluate(target["base"], environment, watch_var, source_code)
            if base_status == "exit": return base, "exit"
            ...
            if index_ast["tag"] == "string":
                ...
                index, index_status = evaluate(index_ast, environment, watch_var, source_code)
                if index_status == "exit": return index, "exit"
        ...
        value, value_status = evaluate(ast["value"], environment, watch_var, source_code)
        if value_status == "exit": return value, "exit"

        target_base[target_index] = value
        
        # Check if watching this variable
        if watch_var and target["tag"] == "identifier" and target["value"] == watch_var:
            line_info = ""
            if "position" in target and source_code:
                # Calculate line number from position
                position = target["position"]
                line_num = source_code[:position].count('\n') + 1
                line_info = f" (line {line_num})"
            elif "position" in target:
                line_info = f" (position {target['position']})"
            print(f"[[ Watched Variable Changed! ]] {watch_var} = {value}{line_info}")
        
        return value, None

    if ast["tag"] == "return":
        if "value" in ast and ast["value"] is not None: # Checks if 'return' has an expression
            evaluated_value, expression_status = evaluate(ast["value"], environment, watch_var, source_code)
    ...
    if ast["tag"] == "exit":
        ...
        if "value" in ast and ast["value"] is not None:
            exit_code_val, status = evaluate(ast["value"], environment, watch_var, source_code)
    ...
    if ast["tag"] == "import":
        filename_val, status = evaluate(ast["value"], environment, watch_var, source_code)
        ...
        try:
            ...
            return evaluate(imported_ast, environment, watch_var, source_code) # Propagates value and status from imported code
...
def equals(code, environment, expected_result, expected_environment=None):
    result, status = evaluate(parse(tokenize(code)), environment, watch_var=None)
...
def test_evaluate_list_literal():
    ...
    result, _ = evaluate(ast, environment, watch_var=None)
    assert result == [1,2,3]
    code = '[]'
    ast = parse(tokenize(code))
    result, _ = evaluate(ast, environment, watch_var=None)
...
def test_evaluate_function_call():
    ...
    result, _ = evaluate(parse(tokenize(code)), environment, watch_var, source_code)
